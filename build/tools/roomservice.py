...
#           (C) 2017-2018,2020-2021, The CygnusOS Project
#
...
if not depsonly:
    print("Device %s not found. Attempting to retrieve device repository from CygnusOS Github (http://github.com/CygnusOS)." % device)
...
if not depsonly:
    githubreq = urllib.request.Request("https://raw.githubusercontent.com/CygnusOS/mirror/main/default.xml")
    try:
...
def add_to_manifest(repositories):
    ...
    for repository in repositories:
        ...
        if is_in_manifest(repo_target):
            print('CygnusOS/%s already fetched to %s' % (repo_name, repo_target))
            continue

        print('Adding dependency: CygnusOS/%s -> %s' % (repo_name, repo_target))
        project = ElementTree.Element("project", attrib = {
            "path": repo_target,
            "remote": "github",
            "name": "CygnusOS/%s" % repo_name,
...
def get_default_or_fallback_revision(repo_name):
    ...
    githubreq = urllib.request.Request("https://api.github.com/repos/CygnusOS/" + repo_name + "/branches")
    add_auth(githubreq)
...
print("Repository for %s not found in the CygnusOS Github repository list. If this is in error, you may need to manually add it to your local_manifests/roomservice.xml." % device)
