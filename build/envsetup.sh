...
cat <<EOF
Additional CygnusOS functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- cygnusgerrit:   A Git wrapper that fetches/pushes patch from/to CygnusOS Gerrit Review.
- cygnusrebase:   Rebase a Gerrit change and push it again.
- cygnusremote:   Add git remote for CygnusOS Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cloremote:       Add git remote for matching CodeLinaro repository.
- githubremote:    Add git remote for CygnusOS Github.
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the Cygnus model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/cygnus-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.cygnus.device | grep -q "$CYGNUS_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $CYGNUS_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function cygnusremote()
{
    ...
    git remote rm cygnus 2> /dev/null
    ...
    local CYGNUS_USER=$(git config --get review.review.cygnusos.org.username)
    if [ -z "$CYGNUS_USER" ]
    then
        git remote add cygnus ssh://review.cygnusos.org:29418/$PFX$PROJECT
    else
        git remote add cygnus ssh://$CYGNUS_USER@review.cygnusos.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'cygnus' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/CygnusOS/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.cygnus.device | grep -q "$CYGNUS_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        cygnusremote
        git push cygnus HEAD:refs/heads/'$1'
    fi
    '
}

function cygnusgerrit() {
    ...
    local user=`git config --get review.review.cygnusos.org.username`
...
function cygnusrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "CygnusOS Gerrit Rebase Usage: "
        echo "      cygnusrebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.cygnusos.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.cygnus.device | grep -q "$CYGNUS_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/cygnus/build/tools/repopick.py $@
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $CYGNUS_FIXUP_COMMON_OUT ]; then
